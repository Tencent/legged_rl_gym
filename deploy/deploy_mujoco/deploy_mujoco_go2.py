...
from unitree_sdk2py.core.channel import ChannelFactoryInitialize   # already imported
from unitree_sdk2py.core.channel import ChannelSubscriber          # already imported
from unitree_sdk2py.idl.unitree_go.msg.dds_ import LowState_ as LowStateGo
from unitree_sdk2py.idl.unitree_go.msg.dds_ import WirelessController_

# ----------  domain / interface  ----------
DOMAIN_ID   = 1          # same as unitree_mujoco.py
INTERFACE   = "lo"       # same as unitree_mujoco.py

ChannelFactoryInitialize(DOMAIN_ID, INTERFACE)     # ← one-line, module level
...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        # ----------  subscribers (non-blocking, no callback) ----------
        ...
        self.latest_low_state = None          # ← 1. buffer
        self.latest_wc        = None 
        self.lowSub = ChannelSubscriber("rt/lowstate", LowStateGo)
        self.lowSub.Init(self.LowStateHandler, 10)   # ← 2. real callback

        self.keys = 0          # 无线控制器 bitmap
    ...
    def WirelessControllerHandler(self, msg: WirelessController_):
        ...
        # === 检测按键信号 ===
        if (self.keys >> 2) & 1:
            print("\033[32m>>> START 键 按下\033[0m")
        if (self.keys >> 8) & 1:
            print("\033[35m>>> A 键 按下\033[0m")
    ...
    def zero_torque_state(self):
        ...
        while not (self.keys >> 2) & 1:    # Check if start button is pressed
    ...
    def default_pos_state(self):
        ...
        while not (self.keys >> 8) & 1:  # Check if A button is pressed
            for i in range(12):
    ...
    def run(self):
        ...
        if self.use_remote_controller:
            self.cmd[0] = self.ly
            self.cmd[1] = self.lx * -1
            self.cmd[2] = self.rx * -1
...
if __name__ == "__main__":
    ...
    while True:
        try:
           controller.run()
           if (controller.keys >> 3) & 1:  # Check if select button is pressed
            break
